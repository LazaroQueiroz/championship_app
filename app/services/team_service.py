...
class TeamService:
    ...
    def list_teams(self) -> list[dict[str, Any]]:
        return sorted(self._load(), key=lambda t: t["name"].lower())
    ...
    def create_team(self, name: str, player_ids: list[str]) -> dict[str, Any]:
        teams = self._load()
        if any(t["name"].strip().lower() == name.strip().lower() for t in teams):
            raise ValueError("Já existe time com esse nome.")
        self._validate_players(player_ids)
        team = {"id": generate_id("tm"), "name": name.strip(), "player_ids": player_ids}
        teams.append(team)
        self._save(teams)
        return team
    ...
    def update_team(self, team_id: str, name: str, player_ids: list[str]) -> dict[str, Any]:
        ...
        for t in teams:
            if t["id"] != team_id and t["name"].strip().lower() == name.strip().lower():
                raise ValueError("Já existe time com esse nome.")
